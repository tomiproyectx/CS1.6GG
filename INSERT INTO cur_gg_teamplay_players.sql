...
SELECT
	...
	ROUND((CAST(REPLACE(raw.HS,'.','') AS REAL)*100)/CAST(REPLACE(raw.HITS,'.','') AS REAL),2) AS headshot_acc_ratio,
	...
	CASE 
		...
		(CASE WHEN INSTR(raw.TIME_PLAYED,',') != 0 THEN CAST(TRIM(SUBSTR(raw.TIME_PLAYED,INSTR(raw.TIME_PLAYED,',')+1,3)) AS INT) ELSE CAST(TRIM(SUBSTR(raw.TIME_PLAYED,1,2)) AS INT)END)
		ELSE 0
	END AS hours_played,
	CASE WHEN INSTR(raw.TIME_PLAYED,'minutos') != 0 OR INSTR(TIME_PLAYED,'minuto') != 0 THEN CAST(TRIM(REPLACE(SUBSTR(raw.TIME_PLAYED,-10,3),'y','')) AS INT) ELSE 0 END AS minutes_played,
